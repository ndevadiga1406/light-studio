...
function cartAlertNo(){
    ...
    let input1=$('.id').filter(function() { return $(this).val() === currentSelection; }).next().children().next();
    input1.val(1); 
    cartLoad()
}
...
function cartAlertYes(){
    let currentSelection=sessionStorage.getItem("currentSelect");
    $('.id').filter(function() { return $(this).val() === currentSelection; }).parent().parent().parent().fadeOut()
    cartLoad()
}
...
function cartLoad(){
    var totalcost= 0;
    const priceBucket= $('h4').children().map(function(){return this.innerHTML;}).get();
    const itemsBucket= $('.total-item').map(function(){return this.value;}).get();
    const calculate =[];
